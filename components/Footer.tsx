...
export default function Footer() {
  return (
    <footer className="bg-white border-t border-[#F3F4F6] pt-6 sm:pt-8 lg:pt-10 pb-8 sm:pb-10 w-full">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 md:px-8 lg:px-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 mb-8">
          {/* LEFT COLUMN: Logo Stack */}
          <div className="flex flex-col items-start">
            <Logo className="mb-1" showTagline={false} />
          </div>

          {/* CENTER COLUMN: Quick Links */}
          <div className="flex flex-col items-start w-full">
            <h4 className="font-extrabold text-[18px] text-[#111827] uppercase tracking-wider mb-3 sm:mb-5">
              Quick Links
            </h4>
            <ul className="grid grid-cols-2 gap-x-6 gap-y-4 w-full">
              {quickLinks.map((link) => (
                <li key={link.label}>
                  <Link
                    href={link.href}
                    className="text-[15px] font-semibold text-[#4B5563] hover:text-[#FF2D3F] transition-colors"
                  >
                    {link.label}
                  </Link>
                </li>
          ...
          <div className="flex flex-col items-start sm:col-span-2 lg:col-span-1">
            <h4 className="font-extrabold text-[18px] text-[#111827] uppercase tracking-wider mb-5">
              Contact Us
            </h4>
            <ul className="space-y-4">
              ...
              <li className="flex items-start gap-3 text-[14px] font-bold text-[#4B5563]">
                ...
                <a
                  ...
                  className="hover:text-[#FF2D3F] transition-colors pt-1"
                >
                  email@urbanatlas.in
                </a>
              </li>
