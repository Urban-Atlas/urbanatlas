...
export default function CtaBanner() {
  return (
    <section className="px-3 sm:px-6 py-6 sm:py-8 bg-white w-full" id="cta-banner">
      <div className="max-w-[1400px] mx-auto relative overflow-hidden rounded-[20px] bg-gradient-to-r from-[#FF2D3F] to-[#E81F34] shadow-[0px_10px_40px_rgba(255,45,63,0.15)]">
        ...
        <div className="relative z-10 px-5 py-8 sm:px-8 sm:py-10 text-center max-w-3xl mx-auto flex flex-col items-center">
          <h2 className="text-[26px] sm:text-[30px] md:text-[40px] font-extrabold text-white mb-2 leading-tight tracking-tight">
            Ready to Sell Your Plot?
          </h2>
          <p className="text-white/90 text-[14px] sm:text-[15px] font-bold mb-7 max-w-lg leading-relaxed">
          ...
          <div className="flex flex-col md:flex-row gap-4 justify-center w-full md:w-auto">
            <MotionLink
              href="/submit-plot"
              className="inline-flex items-center justify-center gap-2 border-2 border-white/60 text-white hover:bg-white/10 rounded-[14px] px-8 py-4 font-black text-[15px] transition-all duration-200 w-full md:w-auto min-h-[52px]"
              whileHover={{ y: -2 }}
              whileTap={{ y: 0 }}
            >
              Submit Your Plot
            ...
            <MotionLink
              href="/buyer-circle"
              className="inline-flex items-center justify-center gap-2 bg-white text-[#111827] rounded-[14px] px-8 py-4 font-extrabold text-[15px] hover:bg-[#F9FAFB] transition-all duration-200 premium-shadow w-full md:w-auto min-h-[52px]"
              whileHover={{ y: -2 }}
              whileTap={{ y: 0 }}
            >
